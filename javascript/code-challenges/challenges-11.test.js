...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map((onlyNums) => {
    let filteredNums = onlyNums.filter((num) => typeof num === "number" && num % 5 === 0);
    return filteredNums.map((num) => 2 ** num);
  });
};
...
let findShortest = (data) => {
  let shortest = data.reduce((acc, curr) => {
    if (
      acc.height === "unknown" ||
      (curr.height !== "unknown" &&
        parseInt(curr.height) < parseInt(acc.height))
    ) {
      return curr;
    }
    return acc;
