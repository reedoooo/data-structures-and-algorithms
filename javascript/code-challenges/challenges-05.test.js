...
const toLastNames = people => {
  let firstLastList = people.map(person => {
    let firstLast = person.firstName + ' ' + person.lastName;
    return firstLast;
  });
...
const addPurchases = (arr) => {
  const totalAmount = arr.reduce((previous, current) => {
    return previous + current.purchasePrice;
  }, 0);
  return totalAmount;
};
...
const countNumberOfElements = (arr) => {
  return arr.reduce((prevEl, currEl) => {
    console.log(currEl);
    return prevEl + 1;
  }, 0);
};
...
const reversedString = (str) => {
  const reverseName = str.split('').reduce((previous, current) => {
